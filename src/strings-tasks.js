...
function removeFirstOccurrences(str, val) {
  return str.includes(val)
    ? `${str.slice(0, str.indexOf(val))}${str.slice(
        str.indexOf(val) + val.length
      )}`
    : str;
}
...
function removeLastOccurrences(str, val) {
  return str.includes(val)
    ? `${str.slice(0, str.lastIndexOf(val))}${str.slice(
        str.lastIndexOf(val) + val.length
      )}`
    : str;
}
...
function sumOfCodes(str) {
  return str
    ? [...str].map((char) => char.charCodeAt()).reduce((sum, cur) => sum + cur)
    : 0;
}
...
function countVowels(str) {
  return str && str.match(/[aeiouy]/gi)
    ? str.match(/[aeiouy]/gi).join('').length
    : 0;
}
...
function isPalindrome(str) {
  const set = str.toLowerCase().replace(/[^a-zа-яё]/gi, '');
  return set === [...set].reverse().join('');
}
...
function extractNameFromTemplate(val) {
  return `${val.slice(val.indexOf(' ') + 1, val.length - 1)}`;
}
...
function encodeToRot13(str) {
  ...
  const result = [...str]
    .map((char) => {
      const i = al.indexOf(char);
      if (i >= 0) {
        if (i < 26) {
          return al[(i + 13) % 26];
        }
        return al[((i + 13) % 26) + 26];
      }
      return char;
    })
    .join('');
